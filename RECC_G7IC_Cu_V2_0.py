...
# Cement for buildings remains, as this item refers to cement in mortar, screed, and plaster. Cement in concrete is included in concrete.
...
#                TotalStockCurves_UsePhase = np.einsum('tGr,tr->trG',RECC_System.ParameterDict['2_S_RECC_FinalProducts_Future'].Values[MS_LED,:,:,:],RECC_System.ParameterDict['2_P_RECC_Population_SSP_32R'].Values[0,:,:,mS])         
        ...
        Inflow_Prod[:,:,mS,mR]      = np.einsum('tgr->tg',Inflow_Detail_UsePhase)
        Outflow_Prod[:,:,mS,mR]     = np.einsum('tcgr->tg',Outflow_Detail_UsePhase)
        ...
        if ScriptConfig['Include_REStrategy_FabYieldImprovement'] == 'True':
            ...
            Par_RECC_MC[115::,8,:,:] = Par_RECC_MC[115::,8,:,:] * (1 - 0.156 * np.einsum('tr,g->tgr',RECC_System.ParameterDict['3_SHA_RECC_REStrategyScaleUp'].Values[:,:,mR]*0.01,np.ones((Ng)))).copy()
        else:
        ...
        SysVar_EnergyDemand_UsePhase_Total_Buildings                = SysVar_EnergyDemand_UsePhase_Total.copy()
        SysVar_EnergyDemand_UsePhase_Total_Buildings[:,:,0:6,:,:,:] = 0
        SysVar_EnergyDemand_UsePhase_Total_Vehicles                 = SysVar_EnergyDemand_UsePhase_Total.copy()
        SysVar_EnergyDemand_UsePhase_Total_Vehicles[:,:,6::,:,:,:]  = 0
        SysVar_EnergyDemand_UsePhase_Buildings_ByEnergyCarrier = np.einsum('Vrnt,tcgrV->trgn',RECC_System.ParameterDict['3_SHA_EnergyCarrierSplit_Buildings'].Values[:,mR,:,:,:],SysVar_EnergyDemand_UsePhase_Total_Buildings[:,:,:,:,-1,:].copy())
        SysVar_EnergyDemand_UsePhase_Vehicles_ByEnergyCarrier  = np.einsum('cgrVn,tcgrV->trgn',RECC_System.ParameterDict['3_SHA_EnergyCarrierSplit_Vehicles'].Values[:,:,:,:,:,mS] ,SysVar_EnergyDemand_UsePhase_Total_Vehicles[:,:,:,:,-1,:].copy())
        ...
        SysVar_TotalEnergyDemand = np.einsum('trgn->tn',SysVar_EnergyDemand_UsePhase_Buildings_ByEnergyCarrier) + np.einsum('trgn->tn',SysVar_EnergyDemand_UsePhase_Vehicles_ByEnergyCarrier) \
