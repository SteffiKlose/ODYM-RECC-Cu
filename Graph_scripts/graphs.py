...
import numpy as np
def figures_flow_all(time, Total_cu_flow, inorout):
  
    fig, axes = plt.subplots(nrows=2, ncols=1, figsize=(16, 8))
    plt.subplot2grid((2,1),(0,0))
    #plt.plot(time_dsm+1900, Total_cu_outflow_app[:,:,:,0,0].sum(axis=0).sum(axis=0),'b')
    plt.plot(time, Total_cu_flow[:,:,:,1,0].sum(axis=0).sum(axis=0),'g')
    plt.plot(time, Total_cu_flow[:,:,:,2,0].sum(axis=0).sum(axis=0),'r')
    #plt.plot(time_dsm+1900, Total_cu_outflow_app[:,:,:,0,1].sum(axis=0).sum(axis=0),'b--')
    plt.plot(time, Total_cu_flow[:,:,:,1,1].sum(axis=0).sum(axis=0),'g--')
    plt.plot(time, Total_cu_flow[:,:,:,2,1].sum(axis=0).sum(axis=0),'r--')
    plt.ylabel('Cu' + inorout+'flow [kt/year]')
